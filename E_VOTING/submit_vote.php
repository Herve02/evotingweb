...
session_start();
if(empty($_POST['lan'])){
$error="<center><h4 class='errs'><font color='#FF0000'>Please select a candidate to vote!</h4></center></font>";
include"voter.php";
exit();
}
$lan = $_POST['lan'];
...
if(mysqli_num_rows($sql) > 0 ) {
	$msg="<center><h4 class='errs'><font color='#FF2000'>You have already been voted, No need to vote again</h4></center></font>";
		include 'voter.php';
		exit();	
}
else{

$sql1 =mysqli_query($con, 'UPDATE candidates SET votecount = votecount + 1 WHERE lan_id = "'.$_POST['lan'].'"');
...
$sql3 = mysqli_query($con, 'UPDATE voters SET voted= "'.$_POST['lan'].'" WHERE username="'.$_SESSION['SESS_NAME'].'"');
	...
	$msg="<center><h4 class='errs'><font color='#FF0000'>Congratulation, you have made your vote.</h4></center></font>";
	include 'voter.php';
	exit();
	}
}
